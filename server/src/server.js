...
app.post('/upload', upload.single('image'), (req, res) => {
    const { serial, name, detail, type1, type2, height, category, gender, weight, characteristic, upload_date, edit_date } = req.body;
    const imagePath = req.file.path; // 이미지 파일 경로

    const sql = "INSERT INTO guidebook (id, serial, name, detail, type1, type2, height, category, gender, weight, characteristic, image, upload_date, edit_date) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    db.query(sql, [serial, name, detail, type1, type2, height, category, gender, weight, characteristic, imagePath, upload_date, edit_date], (err, result) => {
        if (err) {
            console.error("Database query error:", err);
            res.status(500).send("Database query error");
        } else {
